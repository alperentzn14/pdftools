...
class OperationList extends StatelessWidget {
  ...
  _FileCategory _detectCategory() {
    ...
    final extensions = files.map((f) => (f.extension ?? '').toLowerCase()).toSet();
    ...
    switch (cat) {
      case _FileCategory.pdf:
        if (isMulti) {
          return [
            ...
            _OpItem(
              ...
              estSec: '5',
              onTap: () => onDispatch(OpBatchRequest(paths, ConversionType.pdfToWord)),
            ),
          ];
        }
        ...
        return [
          ...
          _OpItem(
            iconWidget: _editIcon(Icons.delete_outline, const Color(0xFFB71C1C)),
          ...
          _OpItem(
            iconWidget: _singleIcon('PDF', const Color(0xFF1A237E), icon: Icons.draw),
        ...
        if (isMulti) {
          return [
            _OpItem(
              ...
              subtitleKey: 'ops.batch_convert_sub',
              estSec: '5',
              onTap: () => onDispatch(OpBatchRequest(paths, ConversionType.wordToPdf)),
            ),
          ];
        }
        return [
...
class OpCompressPdfRequest { final String path; OpCompressPdfRequest(this.path); }
